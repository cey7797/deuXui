...
/**
 * @Class Name : MsSmsDAO.java
 * @Description : MsSms DAO Class
...
@Repository("msSmsDAO")
public class MsSmsDAO extends EgovAbstractDAO {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(ComService.class);
	
	@Resource(name="mssqlSmsSqlMapClient")
	...
	public List<?> selectData(String sqlId, Map map) throws Exception {
		...
		if("en".equals(map.get("gv_langFg"))){
			queryId = MetadataUtil.getSqlIdCheck(getSqlMapClientTemplate(), sqlId + "_en");
		}else{
			queryId = sqlId;	
		}
	...
	public void saveData(String insertQueryId, String updateQueryId, String deleteQueryId, DataSet ds) throws Exception {
		...
		if("en".equals(ds.getString(0, "gvLangFg"))){
			insertSqlId = MetadataUtil.getSqlIdCheck(getSqlMapClientTemplate(), insertQueryId + "_en");
			updateSqlId = MetadataUtil.getSqlIdCheck(getSqlMapClientTemplate(), updateQueryId + "_en");
			deleteSqlId = MetadataUtil.getSqlIdCheck(getSqlMapClientTemplate(), deleteQueryId + "_en");
		}else{
