...
if (!nexacro.Calendar) {
	...
	_pCalendar.on_created_contents = function () {
		...
		switch (this.type) {
			...
			default:
				...
				this._setAccessibilityActiveDescendant(this.calendaredit);
				this.calendaredit.style.set_align(this.currentstyle.align);
				var color = this.on_find_CurrentStyle_color(this._pseudo, this.calendaredit);
				if (color) {
					this.on_apply_style_color(color);
				}
				this.on_apply_style_popupborder(this.currentstyle.popupborder);
		...
		if (propid == "value") {
			...
			if (col_val) {
				if (col_val instanceof nexacro.Date) {
					...
					var add_info = null;
					add_info = this._makeValueStrObj(col_val, true);
	...
	_pCalendar._makeValueStrObj = function (edit_str, _is_fullvalue) {
		...
		if (editformat_info) {
			if (editformat_info._year || editformat_info._mon || editformat_info._day) {
				...
				if (_is_fullvalue && edit_str.length > 9) {
					add_year = edit_str.substr(0, 4);
					add_mon = edit_str.substr(4, 2);
					add_day = edit_str.substr(6, 2);
					value_str = this._makeMaskValue_normal(edit_str);
				}
				else {
					if (edit_str.indexOf(":") > 0) {
						value_str = this._makeNormalValue1(edit_str);
					}
					else {
						value_str = this._makeMaskValue_timeOnly(edit_str);
					}
				}
			}

			if (editformat_info._year && editformat_info._year.index > 0) {
				return {
					full_str : (_is_fullvalue) ? edit_str : value_str, 
					add_year : add_year, 
					add_mon : add_mon, 
					add_day : add_day
				};
			...
			if (!this._isUndefinedDate(value_str) && !this._isNullDate(value_str) && !this._isEmptyStr(value_str)) {
				if (empty_date) {
					add_year = add_year ? add_year : "0000";
					add_mon = add_mon ? add_mon : "01";
					add_day = add_day ? add_day : "01";
				}
				else {
	...
	_pCalendar._makeDateObj = function (v, add_info) {
		...
		if (!add_info) {
			var add_info = this._makeValueStrObj(val);
		}
	...
	_pCalendarEditCtrl.on_create_contents = function () {
		...
		if (control) {
			...
			this.set_enable(this.parent.enable);
		}
	};
